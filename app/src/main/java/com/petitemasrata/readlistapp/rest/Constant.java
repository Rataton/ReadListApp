package com.petitemasrata.readlistapp.rest;

/**
 * Created by irata on 28/04/15.
 */
public class Constant {
    public static final String URL_KEY = "http://api.eventful.com/json/events/search?app_key=gWjFpzHrNbPrt22W&keywords=metal&location=mexico&sort_order=date&category=music";
    public static final String EVENTS_KEY = "events";
    public static final String EVENT_KEY = "event";

    public static final String TITLE_KEY = "title";
    public static final String DESCRIPTION_KEY = "description";
    public static final String DATE_KEY = "start_time";
    public static final String EVENT_URL_KEY = "url";

    public static final String VENUE_KEY = "venue_name";
    public static final String ADDRESS_KEY = "venue_address";
    public static final String CITY_KEY = "city_name";
    public static final String REGION_KEY = "region_name";
    public static final String COUNTRY_KEY = "country_name";

    public static final String TXT_DE = "de";
    public static final String TXT_HORAS = "horas";

}
